...
app.post('/api/message', async (req, res) => {
    try {
        ...
        // If conversationId is 'new', check if a conversation already exists between the users
        let conversation = null;
        if (conversationId === 'new' && receiverId) {
            conversation = await Conversations.findOne({
                members: { $all: [senderId, receiverId] } // Check if conversation between these users exists
            });
            ...
            return res.status(400).send('Please provide a valid conversationId or receiverId');
        } else {
